...
struct _NautilusWindowInfoIface 
{
	...
	/* return TRUE if other pane is writable (files can be moved/copied there),
	 * FALSE if it is not writable or no other pane exists */
	gboolean (* next_pane_is_writable) (NautilusWindowInfo *window);    
    void (* copy_move_selection_to_next_pane) (NautilusWindowInfo *window, gboolean want_copy);
